...
const labels={ja:{intro:'すべて必須項目です。入力内容を確認してから、サイト上で送信できます。',notice:'入力後に内容確認画面が表示されます。確認後、このサイトから直接送信されます。',sending:'送信中…',failed:'送信に失敗しました。時間をおいてもう一度お試しください。',confirm:'この内容で送信しますか？',emailLabel:'メールアドレス',invalidEmail:'有効なメールアドレスを入力してください。例: name@example.com',notConfigured:'送信先が設定されていません。現在このフォームからは送信できません。',stagingMismatch:'STAGINGの安全確認に失敗しました。送信完了として扱いません。',platformLabel:'希望するネイティブ対応',sourceLabel:'問い合わせ経路',webDemoSource:'Web Demo'},zh:{intro:'所有字段均为必填。您可以在从本网站发送前确认咨询内容。',notice:'填写后将显示内容确认页面。确认后会直接从本网站发送。',sending:'发送中…',failed:'发送失败，请稍后重试。',confirm:'发送此咨询吗？',emailLabel:'电子邮箱',invalidEmail:'请输入有效的电子邮箱，例如 name@example.com。',notConfigured:'尚未配置提交地址，目前无法通过此表单发送。',stagingMismatch:'STAGING 安全检查失败，本次提交不会视为已完成。',platformLabel:'希望支持的原生平台',sourceLabel:'咨询来源',webDemoSource:'网页演示'},en:{intro:'All fields are required. Review your inquiry before sending it directly from this website.',notice:'After entering your details, you can review them before the inquiry is sent directly from this website.',sending:'Sending…',failed:'The message could not be sent. Please try again later.',confirm:'Send this inquiry?',emailLabel:'Email address',invalidEmail:'Enter a valid email address, for example name@example.com.',notConfigured:'The submission endpoint is not configured. This form cannot send at the moment.',stagingMismatch:'The staging safety check failed. This submission will not be treated as complete.',platformLabel:'Requested native platform',sourceLabel:'Inquiry source',webDemoSource:'Web Demo'}};
...
function showConfirm(){const form=$('#contactForm'),confirm=$('#inquiryConfirm'),complete=$('#inquiryComplete');if(!form||!confirm)return;renderSummary();form.hidden=true;if(complete)complete.hidden=true;confirm.hidden=false;confirm.querySelector('h2').textContent=tr('confirm');$('#directSendNotice').textContent=tr('notice');ensureTurnstileRuntime().catch(error=>console.warn('Turnstile preload failed',error));confirm.scrollIntoView({behavior:'smooth',block:'start'});const heading=confirm.querySelector('h2');heading.tabIndex=-1;heading.focus({preventScroll:true})}
function showForm(){const form=$('#contactForm'),confirm=$('#inquiryConfirm');if(confirm)confirm.hidden=true;if(form){form.hidden=false;form.scrollIntoView({behavior:'smooth',block:'start'});form.querySelector('input')?.focus({preventScroll:true})}}
function workerPayload(){const context=[];if(SOURCE==='web-demo')context.push('[Source: Web Demo]');if(PLATFORM_INTEREST)context.push(`[Native platform interest: ${platformDisplay()}]`);const message=[...context,value('message')].filter(Boolean).join('\n');return{type:'inquiry',name:value('name'),company:value('company'),country:value('country'),email:value('contact'),product:value('product'),message,lang:lang(),website:''}}
