...
function renderNews(){
 ...
 const latest=document.getElementById('cmsLatestList');if(latest){latest.innerHTML='<h2>Latest</h2>'+all.slice(0,5).map(n=>`<a class="latest-row" href="${attr(n.link||'#')}"><small>${esc(dateLabel(n.date))} / ${esc(n.category||'Update')}</small><strong>${esc(pick(n.title))}</strong></a>`).join('');}
 const wire=document.getElementById('cmsNewsWire');if(wire){wire.innerHTML=all.map(n=>`<a class="wire-row" href="${attr(n.link||'#')}"${(n.link||'').includes('demo.html')?' data-demo-open':''}><div class="wire-type">${esc(n.category||'Update')}</div><div><h3>${esc(pick(n.title))}</h3><p>${esc(pick(n.body))}</p></div><time>${esc(dateLabel(n.date))}</time></a>`).join('');}
 renderInstagram();
}
...
async function load(){try{const r=await fetch(`content/site-content.json?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(r.status);CMS=await r.json();renderStrip();renderNews();renderInstagram();}catch(e){console.warn('CMS content load failed',e)}}
