...
function startStripAuto(){
 clearInterval(stripTimer);stripTimer=null;
 if(STRIP_ITEMS.length>1)stripTimer=setInterval(()=>setStripItem(stripIndex+1,true),5000);
}
function renderStrip(){
 const bar=document.querySelector('.news-strip');if(!bar||!CMS)return;
 STRIP_ITEMS=categoryLatest();
 if(!STRIP_ITEMS.length){bar.hidden=true;return}
 bar.hidden=false;const a=bar.querySelector('a');if(a)a.href='news.html';
 stripIndex=Math.min(stripIndex,STRIP_ITEMS.length-1);setStripItem(stripIndex,false);startStripAuto();
}
...
function goLatest(index,smooth=true){
 if(!LATEST_ITEMS.length)return;
 latestIndex=(index+LATEST_ITEMS.length)%LATEST_ITEMS.length;
 const viewport=document.querySelector('.news-latest-viewport');
 const cards=viewport?.querySelectorAll('.latest-card');
 const card=cards?.[latestIndex];if(!viewport||!card)return;
 viewport.scrollTo({left:card.offsetLeft,behavior:smooth?'smooth':'auto'});
 document.querySelectorAll('.latest-dot').forEach((d,i)=>d.classList.toggle('active',i===latestIndex));
}
function startLatestAuto(){
 clearInterval(latestTimer);latestTimer=null;
 if(LATEST_ITEMS.length>1)latestTimer=setInterval(()=>goLatest(latestIndex+1,true),5000);
}
function renderLatest(all){
 const latest=document.getElementById('cmsLatestList');if(!latest)return;
 LATEST_ITEMS=categoryLatest(all);latestIndex=0;
 if(!LATEST_ITEMS.length){latest.innerHTML='';return}
 latest.innerHTML=`<div class="latest-carousel-head"><div><small>Latest stories</small><strong>Explore updates</strong></div><div class="latest-carousel-controls"><button type="button" data-latest-prev aria-label="Previous news">←</button><button type="button" data-latest-next aria-label="Next news">→</button></div></div><div class="news-latest-viewport"><div class="news-latest-track">${LATEST_ITEMS.map(n=>`<button class="latest-card news-open" type="button" data-news-id="${attr(n.id)}">${newsImage(n,'cms-latest-image')}<div class="latest-card-copy"><small>${esc(n.category||'Update')} / ${esc(dateLabel(n.date))}</small><strong>${esc(pick(n.title))}</strong><span>Read story →</span></div></button>`).join('')}</div></div><div class="latest-dots">${LATEST_ITEMS.map((_,i)=>`<button class="latest-dot${i===0?' active':''}" type="button" data-latest-index="${i}" aria-label="Go to news ${i+1}"></button>`).join('')}</div>`;
 const viewport=latest.querySelector('.news-latest-viewport');
 latest.addEventListener('mouseenter',()=>{clearInterval(latestTimer);latestTimer=null},{once:false});
 latest.addEventListener('mouseleave',startLatestAuto,{once:false});
 viewport?.addEventListener('focusin',()=>{clearInterval(latestTimer);latestTimer=null});
 viewport?.addEventListener('focusout',startLatestAuto);
 startLatestAuto();
}
function renderNews(){
 ...
 if(lead){lead.href='#';lead.dataset.newsId=featured.id;lead.innerHTML=`${newsImage(featured,'cms-lead-image')}<div class="news-lead-copy"><div class="news-label">Top News / ${esc(featured.category||'Update')}</div><h2>${esc(pick(featured.title))}</h2><p>${esc(pick(featured.body))}</p><div class="news-meta">${esc(dateLabel(featured.date))}</div></div>`;}
 renderLatest(all);
...
document.addEventListener('click',e=>{
 ...
 const open=e.target.closest('[data-news-id]');if(open&&document.body.classList.contains('cms-news-page')){e.preventDefault();openArticle(open.dataset.newsId);return}
 if(e.target.closest('[data-news-close]'))closeArticle();
});
