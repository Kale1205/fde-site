...
function openArticle(id){const n=(CMS?.news||[]).find(x=>x.id===id);if(!n)return;const modal=ensureArticleModal(),content=modal.querySelector('#newsArticleContent');content.innerHTML=`<div class="news-article-meta">${esc(dateLabel(n.date))} / ${esc(n.category||'Update')}</div><h2 id="newsArticleTitle">${esc(pick(n.title))}</h2>${newsImage(n,'news-article-image')}<div class="news-article-body">${esc(pick(n.body)).replace(/\n/g,'<br>')}</div>`;modal.hidden=false;document.body.classList.add('news-modal-open');modal.querySelector('.news-article-close')?.focus()}
...
function goLatest(index,smooth=true){if(!LATEST_ITEMS.length)return;latestIndex=(index+LATEST_ITEMS.length)%LATEST_ITEMS.length;const viewport=document.querySelector('.news-latest-viewport');const cards=viewport?.querySelectorAll('.latest-card');const card=cards?.[latestIndex];if(!viewport||!card)return;viewport.scrollTo({left:card.offsetLeft,behavior:smooth?'smooth':'auto'});document.querySelectorAll('.latest-dot').forEach((d,i)=>d.classList.toggle('active',i===latestIndex))}
function startLatestAuto(){clearInterval(latestTimer);latestTimer=null;if(LATEST_ITEMS.length>1)latestTimer=setInterval(()=>goLatest(latestIndex+1,true),5000)}
function renderLatest(all){const latest=document.getElementById('cmsLatestList');if(!latest)return;LATEST_ITEMS=categoryLatest(all);latestIndex=0;if(!LATEST_ITEMS.length){latest.innerHTML='';return}latest.innerHTML=`<div class="latest-carousel-head"><div><small>Latest stories</small><strong>Explore updates</strong></div><div class="latest-carousel-controls"><button type="button" data-latest-prev aria-label="Previous news">←</button><button type="button" data-latest-next aria-label="Next news">→</button></div></div><div class="news-latest-viewport"><div class="news-latest-track">${LATEST_ITEMS.map(n=>`<button class="latest-card news-open" type="button" data-news-id="${attr(n.id)}">${newsImage(n,'cms-latest-image')}<div class="latest-card-copy"><small>${esc(n.category||'Update')} / ${esc(dateLabel(n.date))}</small><strong>${esc(pick(n.title))}</strong><span>Read story →</span></div></button>`).join('')}</div></div><div class="latest-dots">${LATEST_ITEMS.map((_,i)=>`<button class="latest-dot${i===0?' active':''}" type="button" data-latest-index="${i}" aria-label="Go to news ${i+1}"></button>`).join('')}</div>`;const viewport=latest.querySelector('.news-latest-viewport');latest.addEventListener('mouseenter',()=>{clearInterval(latestTimer);latestTimer=null},{once:false});latest.addEventListener('mouseleave',startLatestAuto,{once:false});viewport?.addEventListener('focusin',()=>{clearInterval(latestTimer);latestTimer=null});viewport?.addEventListener('focusout',startLatestAuto);startLatestAuto()}
