(() => {
  ...
  function newsImage(item, className) {
    if (item?.image) {
      return `<div class="${className}"><img src="${attr(item.image)}" alt="${attr(pick(item.title))}" loading="lazy"></div>`;
    }
  ...
  function ensureArticleModal() {
    ...
    modal.innerHTML = '<div class="news-article-backdrop" data-news-close></div><article class="news-article-panel"><button class="news-article-close" type="button" aria-label="Close story" data-news-close>×</button><div id="newsArticleContent"></div></article>';
  ...
  function renderLatest(items) {
    ...
    if (!latest) return;
    if (!items.length) {
      latest.innerHTML = '<p class="news-empty">No additional updates yet.</p>';
      return;
    }
    latest.innerHTML = items.map(item => `<button class="latest-card news-open" type="button" data-news-id="${attr(item.id)}">${newsImage(item, 'cms-latest-image')}<div class="latest-card-copy"><small>${esc(item.category || 'Update')} / ${esc(dateLabel(item.date))}</small><strong>${esc(pick(item.title))}</strong><span>Read story →</span></div></button>`).join('');
  }
  ...
  function renderArchive(items) {
    ...
    if (!wire) return;
    if (!items.length) {
      wire.innerHTML = '<p class="news-empty">No archived updates yet.</p>';
      return;
    }
  ...
  function renderInstagram() {
    ...
    const media = data.image
      ? `<div class="instagram-photo"><img src="${attr(data.image)}" alt="${attr(data.handle || 'Instagram')}"></div>`
    ...
    box.innerHTML = `${media}<div class="instagram-copy"><small>Instagram / Creative</small><h2>${esc(data.handle || 'Instagram')}</h2><p>${esc(pick(data.description))}</p></div><div class="instagram-cta">View Instagram ↗</div>`;
  }
  ...
  function renderNews() {
    ...
    if (lead) {
      ...
      lead.innerHTML = `${newsImage(featured, 'cms-lead-image')}<div class="news-lead-copy"><div class="news-label">Top News / ${esc(featured.category || 'Update')}</div><h2>${esc(pick(featured.title))}</h2><p>${esc(pick(featured.body))}</p><div class="news-meta">${esc(dateLabel(featured.date))}</div><span class="news-read-more">Read story →</span></div>`;
    }
  ...
  async function load() {
    try {
      ...
      console.warn('CMS content load failed', error);
    }
  }
