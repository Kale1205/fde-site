...
if(/(?:^|\/)(?:contact|order)\.html$/.test(location.pathname) && !window.__FDE_TURNSTILE_LOADER_ADDED__){
  ...
  turnstileScript.src=`${FDE_SITE_BASE}turnstile-protection.js?v=20260820-081040`;
...
if(location.pathname.endsWith('/cms-admin.html') && !window.__FDE_ADMIN_RUNTIME_LOADED__){
  ...
  const syncAdminKeyFields=value=>{
    const key=String(value??readSessionKey()).trim();
    if(!key)return;
    KEY_SELECTORS.forEach(sel=>{const el=document.querySelector(sel);if(el&&el.value!==key)el.value=key});
  };
  window.FDE_ADMIN_KEY={
    get:readSessionKey,
    set(value){const key=String(value||'').trim();writeSessionKey(key);syncAdminKeyFields(key);return key},
    clear(){writeSessionKey('');KEY_SELECTORS.forEach(sel=>{const el=document.querySelector(sel);if(el)el.value=''})}
  };
  ...
  document.addEventListener('input',e=>{
    ...
    if(KEY_SELECTORS.some(sel=>el.matches(sel))){
      const key=el.value.trim();
      writeSessionKey(key);
      if(key)syncAdminKeyFields(key);
    }
  },true);
  ...
  function watchStatus(){
    const el=document.getElementById('adminStatus');if(!el||el.dataset.toastBound)return;
