...
async function generate(type,button){
 ...
 try{
  ...
  const map={ADMIN_AUTH_FAILED:'Admin fulfillment keyが一致しません。',ADMIN_ORDER_NOT_FOUND:'この注文は顧客台帳機能追加前の旧注文、または顧客台帳データが存在しない注文です。新規注文で確認してください。',PAYMENT_NOT_CONFIRMED:'納品書・領収書は「支払い確認済み」以降に生成できます。',BROWSER_NOT_CONFIGURED:'Cloudflare Browser RunのBROWSER bindingがまだ本番Workerへ反映されていません。',ORDER_STATUS_NOT_CONFIGURED:'Cloudflare KV（ORDER_STATUS）が有効になっていません。'};message(map[e.message]||`PDFを生成できませんでした: ${e.message}`,'error')
