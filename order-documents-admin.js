...
function syncLegacyState(){
 const card=$('#orderDocumentsCard');if(!card)return;
 const legacy=isLegacy();
 card.querySelectorAll('[data-doc]').forEach(btn=>{btn.disabled=legacy;btn.setAttribute('aria-disabled',legacy?'true':'false')});
 const note=$('#orderDocumentsLegacyNote');if(note)note.hidden=!legacy;
 const msg=$('#orderDocumentsMessage');if(msg&&legacy){msg.textContent='この注文は顧客台帳機能追加前の旧注文のため、正式なPDFは生成できません。新しいテスト注文または今後の注文で確認してください。';msg.className='status';msg.hidden=false}else if(msg&&!legacy&&msg.textContent.includes('旧注文'))msg.hidden=true;
}
async function generate(type,button){
 ...
 try{
   ...
   const map={ADMIN_AUTH_FAILED:'Admin fulfillment keyが一致しません。',ADMIN_ORDER_NOT_FOUND:'この注文は顧客台帳機能追加前の旧注文、または顧客台帳データが存在しない注文です。新規注文で確認してください。',PAYMENT_NOT_CONFIRMED:'納品書・領収書は「支払い確認済み」以降に生成できます。',BROWSER_NOT_CONFIGURED:'Cloudflare Browser RunのBROWSER bindingがまだ本番Workerへ反映されていません。',ORDER_STATUS_NOT_CONFIGURED:'Cloudflare KV（ORDER_STATUS）が有効になっていません。'};
...
function inject(){
 ...
 const legacy=$('#orderAdminLegacy');if(legacy)new MutationObserver(syncLegacyState).observe(legacy,{attributes:true,attributeFilter:['hidden']});
 const detailObserver=new MutationObserver(()=>setTimeout(syncLegacyState,0));detailObserver.observe(detail,{attributes:true,attributeFilter:['data-order-id']});
 setTimeout(syncLegacyState,0);
 return true;
}
