...
function documentHtml(type,order,status){
 ...
 const amountLabel=isReceipt?'領収金額 / Amount received':'注文金額 / Order total';
 const note=isQuote
 ...
 <table class="table"><thead><tr><th>内容 / Description</th><th>契約 / Plan</th><th style="text-align:right">金額 / Amount</th></tr></thead><tbody><tr><td><b>${esc(order.product)}</b><br>Kale’s FDE software product</td><td>${esc(planLabel(order.plan))}</td><td class="amount">${esc(order.price)}</td></tr></tbody></table>
 <div class="total"><div class="totalbox"><span>${esc(amountLabel)}</span><strong>${esc(order.price)}</strong></div></div>
