...
async function writeOrderUpdate(env,raw){if(!env.ORDER_STATUS)throw new Error('ORDER_STATUS_NOT_CONFIGURED');const orderId=clean(raw.orderId,40).toUpperCase();if(!validOrderId(orderId))throw new Error('INVALID_ORDER_ID');const admin=await readJson(env,`admin-order:${orderId}`),statusRecord=await readJson(env,`order:${orderId}`);if(!admin||!statusRecord)throw new Error('ORDER_NOT_FOUND');const status=clean(raw.status,50);if(status&&!ORDER_STATUSES.has(status))throw new Error('INVALID_STATUS_DATA');if(status)statusRecord.status=status;if(Object.prototype.hasOwnProperty.call(raw,'message'))statusRecord.message=clean(raw.message,1200);statusRecord.updatedAt=new Date().toISOString();await env.ORDER_STATUS.put(`order:${orderId}`,JSON.stringify(statusRecord));return{admin,status:statusRecord}}
...
export default{
 async fetch(request,env,ctx){
  ...
  if(type==='admin_order_update'){
   try{const updated=await writeOrderUpdate(env,raw);return json({ok:true,status:updated.status.status,message:updated.status.message,updatedAt:updated.status.updatedAt},200,origin,allowedOrigin)}catch(error){const code=String(error?.message||'ORDER_UPDATE_FAILED');const status=code==='ORDER_NOT_FOUND'?404:code==='ORDER_STATUS_NOT_CONFIGURED'?503:400;return json({ok:false,error:code},status,origin,allowedOrigin)}
  }
