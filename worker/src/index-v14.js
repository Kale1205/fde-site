...
async function enrichLocale(env,q,a,locale){const language=LOCALE_NAMES[locale]||locale;const pq=protect(q),pa=protect(a),tokenPairs=[...pq.tokens,...pa.tokens];const preserve='Preserve tokens such as FDETERM0X exactly and do not translate brand/product names represented by those tokens.';const sourceNote=locale==='ja'?'Keep question and answer exactly in Japanese as provided.':'Translate question and answer naturally into the target language.';const prompt=`You are preparing FAQ search data for a professional B2B software website. Target language: ${language}. ${sourceNote} ${preserve}\n\nJapanese question:\n${pq.value}\n\nJapanese answer:\n${pa.value}\n\nReturn ONLY valid JSON with exactly these fields:\n{"question":"...","answer":"...","keywords":["..."],"synonyms":["..."]}\n\nRules:\n- keywords: 5 to 8 short search terms a customer may type in ${language}.\n- synonyms: 4 to 7 alternative words or natural phrasings with the same user intent in ${language}; they may be phrases rather than strict dictionary synonyms.\n- Include common abbreviations or product-related wording when relevant.\n- Do not invent product policies, prices, features, promises, supported platforms, or payment methods that are not in the source.\n- No markdown and no commentary.`;
 try{const out=await retry(()=>env.AI.run('@cf/meta/llama-3.1-8b-instruct-fast',{prompt,max_tokens:1300,temperature:.1}));const parsed=extractJson(extractText(out));const question=locale==='ja'?q:restore(clean(parsed.question,1200),tokenPairs),answer=locale==='ja'?a:restore(clean(parsed.answer,7000),tokenPairs);if(!question||!answer)throw new Error('AI_FAQ_FIELDS_EMPTY');const keywords=terms(parsed.keywords).map(x=>restore(x,tokenPairs)),synonyms=terms(parsed.synonyms).map(x=>restore(x,tokenPairs));return{question,answer,keywords,synonyms}}catch(error){console.warn(`FAQ enrichment fallback for ${locale}`,error);return fallbackLocale(env,q,a,locale)}
}
